...
用户选择预算后，网页 GPT 必须先在控制Issue中创建一条纯JSON评论，再提交 `execute_team`。模板：

```text
budget_approval_receipt.template.json
```
...
```json
{
  "operation_id": "本次execute_team的operation_id",
  "operation": "execute_team",
  "deepseek_assist_operation_id": "前置ASSIST的operation_id",
  "budget_approval": {
    "approval_status": "approved_by_user",
    "tier": "economy | balanced | quality | custom",
    "currency": "USD",
    "max_cost_usd": 2.0,
    "estimated_cost_usd": {"low": 0.25, "high": 1.5},
    "max_model_calls": 4,
    "max_output_tokens_per_call": 1200,
    "approval_reference": "用户真实选择的简短记录"
  },
  "source": "web_gpt_after_user_budget_selection"
}
```

`receipt_comment_id` 对 `execute_team` 是强制字段。服务器会读取评论正文并核对：

- 回执与当前 `operation_id` 一致；
- 回执指向相同的DeepSeek ASSIST；
- 回执预算与Execution Plan逐字段完全一致；
- 来源标记证明回执是在用户选择之后由网页 GPT 创建；
- DeepSeek ASSIST的持久结果真实存在、状态为 `READY`、包含三档预算和预算询问。

任何不一致都必须在付费调用前停止。普通模型情报和DeepSeek自身操作仍可使用服务器端自动回执。
...
只有第一类可以自动产生最小代码修改。第二至第五类不得伪造仓库修复。DeepSeek不得修改或伪造用户预算回执。
...
> DeepSeek先接触、预算先展示、用户先批准、回执先落盘、工具临时插入、执行全程可审计、结果必须复核、故障统一交给DeepSeek、上游维护归上游。
