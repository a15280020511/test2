...
class PermanentStatusControlPlaneTests(unittest.TestCase):
    ...
    def test_repair_and_retry_statuses_are_explicit(self) -> None:
        ...
        self.assertEqual(repairing["repair_status"], "repairing")
        self.assertEqual(retrying["status"], "retrying")
        self.assertEqual(retrying["repair_status"], "attempted")
    ...
    def test_final_stop_preserves_failed_operation_semantics(self) -> None:
        old_cwd = Path.cwd()
        with tempfile.TemporaryDirectory() as tmp:
            os.chdir(tmp)
            try:
        ...
        self.assertFalse(payload["result_ready"])
        self.assertEqual(payload["repair_status"], "attempted")
